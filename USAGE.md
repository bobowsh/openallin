...
2. 检测到你在使用 OpenCode，开始安装...
   ✅ 创建 .opencode/ 目录
   ✅ 安装 23 个命令（oa-propose, oa-brainstorming, oa-tdd, oa-security, oa-review 等）
   ✅ 复制 agents/（角色定义）
   ✅ 复制 rules/（编码规范）
...
| 你说的 | AI 做的事 |
|--------|----------|
| "检查安全性" | 调用 `/oa-security` |
| "安全审计" | 调用 `/oa-security` |
| "这个代码有安全漏洞吗？" | 调用 `/oa-security` + 分析 |
| "review 一下安全性" | OWASP + STRIDE 检查 |
...
| 你说的 | AI 做的事 |
|--------|----------|
| "浏览器测试" | 调用 `/oa-qa-browser` |
| "视觉回归测试" | 调用 `/oa-qa-browser` + 视觉检查 |
| "无障碍测试" | 调用 `/oa-qa-browser` + WCAG 检查 |
| "性能测试" | 调用 `/oa-benchmark` |
| "API 性能基准" | 调用 `/oa-benchmark` + API 基准 |

#### 审查相关

| 你说的 | AI 做的事 |
|--------|----------|
| "review 代码" | 调用 `/oa-review` |
| "代码审查" | 调用 `/oa-review` |
| "设计审查" | 调用 `/oa-review` + 设计检查 |
| "架构审查" | 调用 `/oa-review` + 架构检查 |
| "审查 UI/UX" | 调用 `/oa-review` + UI/UX 检查 |

#### 设计相关

| 你说的 | AI 做的事 |
|--------|----------|
| "设计 UI" | 调用 `/oa-ui-design` |
| "生成设计系统" | 调用 `/oa-ui-design` + 输出设计系统 |
| "配色方案" | 调用 `/oa-ui-design` + 颜色推荐 |
| "设计着陆页" | 调用 `/oa-ui-design` + 着陆页模式 |
| "UI 风格推荐" | 调用 `/oa-ui-design` + 风格推荐 |
| "字体搭配" | 调用 `/oa-ui-design` + 字体推荐 |
...
3. **阅读 QUICKREF.md** - 快速参考卡（23 个命令速查）
